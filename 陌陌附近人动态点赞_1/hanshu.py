...
def shaixuanfujin():
    ...
    # # 点击附近
    ###附近控件位置（修改）
    Selector().type("FrameLayout").id(
        "com.immomo.momo:id/nearby_people_filter"
    ).click().find_all()
    ...
    ###男生控件位置
    res_man_yxz = FindImages.find_all_template(
        [
            R.img("4.png"),
        ],
        confidence=0.95,
    ...
    if l_res_man_ydz != 0:
        ...
        res_man = FindImages.find_all_template(
            [
                R.img("2.png"),
            ],
            confidence=0.95,
...
def dianjitouxiang():
    ...
    time.sleep(3)
    res_ydz = FindImages.find_all_template(
        [
            R.img("3.png"),
        ],
        confidence=0.95,
    ...
    if l_res_ydz == 0:
        res_dz = FindImages.find_all_template(
            [
                R.img("1.png"),
            ],
            confidence=0.95,
        ...
        if l_res_dz != 0:
            ...
            print("已点赞，等待退出")
            time.sleep(5)
            Selector().type("ImageView").id(
                "com.immomo.momo:id/iv_back"
            ).click().find_all()
        else:
            print("无动态，等待退出")
            time.sleep(3)
